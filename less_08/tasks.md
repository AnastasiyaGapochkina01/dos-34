...
Best regards.
```
### Работа с пользователями (повторение)
...
7) Переключиться на пользователя `designer` и проверить доступ к директории `/opt/srv/nginx` - создать в ней файл `server.conf` с любым содержимым
...
### Работа с systemd (повторение)
1) Запустить приложение из репозитория https://gitlab.com/devops201206/orange-buttons# с помощью systemd.
2) Запустить приложение из репозитория https://gitlab.com/devops201206/simple-http-server.git с помощью systemd. Требования
  ...
  - Код приложения расположен в директории `/opt/srv/webapp`
